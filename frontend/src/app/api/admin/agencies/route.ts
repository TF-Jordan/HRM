...
export async function GET() {
  return requirePermissionRoute("organizations:write", async (session) => {
    ...
    const data = await orgApi.listAgencies(orgId, session);
...
export async function POST(request: NextRequest) {
  return requirePermissionRoute("organizations:write", async (session) => {
    ...
    const body = (await request.json()) as orgApi.CreateAgencyRequest;
    const data = await orgApi.createAgency(orgId, body, session);
