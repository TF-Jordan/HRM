...
export async function GET(request: NextRequest) {
  return requirePermissionRoute("hrm:payroll:read", async (session) => {
    ...
    const data = await payrollApi.listTaxBracketTables(session, countryCode);
...
export async function POST(request: NextRequest) {
  return requirePermissionRoute("hrm:payroll:run", async (session) => {
    const body = (await request.json()) as Partial<payrollApi.CreateTaxBracketTableRequest>;
    ...
    if (!Array.isArray(body.brackets) || body.brackets.length === 0) {
      return fail(400, "INVALID_BODY", "At least one bracket is required.");
    }
    const payload: payrollApi.CreateTaxBracketTableRequest = {
      code: body.code,
      label: body.label,
    ...
    const data = await payrollApi.createTaxBracketTable(payload, session);
