...
export async function GET(
  ...
  return requirePermissionRoute("hrm:review:read", async (session) => {
    const data = await reviewsApi.listObjectives(id, session);
...
export async function POST(
  ...
  return requirePermissionRoute("hrm:review:manage", async (session) => {
    const body = (await request.json()) as reviewsApi.AddObjectiveRequest;
    const data = await reviewsApi.addObjective(id, body, session);
