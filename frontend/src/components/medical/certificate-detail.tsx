...
import { alertLevel, alertTone } from "@/lib/medical-status";
import type { MedicalCertificateResponse } from "@/server/ksm/modules/medical";
...
export function CertificateDetail({ certificateId }: { certificateId: string }) {
  ...
  const query = useQuery({
    queryKey: ["hrm", "medical", "certificate", certificateId],
    queryFn: () =>
      apiFetch<MedicalCertificateResponse>(`/api/hrm/medical/certificates/${certificateId}`),
  });
  ...
  const reference = `CR-${c.id.slice(0, 4).toUpperCase()}-${c.id.slice(4, 8).toUpperCase()}`;
      ...
      <PageHeader
        ...
        subtitle={
          <span className="flex flex-wrap items-center gap-3">
            ...
            <span className="text-[12.5px] text-ink-3">{c.statut}</span>
          </span>
        }
        actions={
          <Link href="/medical">
      ...
      <Card>
        <CardContent padding="lg">
          <dl className="grid grid-cols-2 gap-x-8 gap-y-4 md:grid-cols-4">
            <Detail label={t("certificates.columns.type")} value={c.typeCertificat} />
            <Detail
              label={t("certificates.columns.emission")}
              value={formatDate(c.dateEmission, { locale })}
              mono
            />
            <Detail
              label={t("certificates.columns.expiration")}
              value={
              ...
              mono
            />
            <Detail label={t("certificates.columns.statut")} value={c.statut} />
            <Detail
              label={t("detail.file")}
              value={
                c.fichierId ? (
          ...
          <p className="mt-6 text-[12px] text-ink-4">
            ID · <span className="font-mono-tabular">{c.id}</span>
          </p>
        </CardContent>
      </Card>
    </>
