...
public final class BillingOperationsRequests {
    ...
    public record SyncDocumentToAccountingRequest(
            String invoiceNumber,
            Boolean postInvoice) {
    }

    public record RecordCashierPaymentRequest(
            @NotNull @DecimalMin(value = "0.01") BigDecimal amount,
            UUID sessionId,
            UUID registerId,
            String reference,
            Boolean syncAccountingSettlement) {
    }

    public record RecordBankSettlementRequest(
