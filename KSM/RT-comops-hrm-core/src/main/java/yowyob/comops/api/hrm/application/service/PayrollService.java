...
public class PayrollService implements RunPayrollUseCase {
    ...
    public Mono<PayrollRun> runPayroll(String periode, UUID agencyId) {
        return ReactiveRequestContextHolder.getRequiredContext()
                .flatMap(context -> executePayrollCore(
                        context.tenantId(), context.organizationId(), agencyId, periode));
    }

    @Override
    public Mono<PayrollRun> autoRunPayroll(UUID tenantId, UUID orgId, UUID agencyId, String periode) {
        return executePayrollCore(tenantId, orgId, agencyId, periode);
    }

    private Mono<PayrollRun> executePayrollCore(UUID tenantId, UUID orgId, UUID agencyId, String periode) {
        ...
        return existsCheck.flatMap(exists -> {
            ...
            return employees.collectList().flatMap(empList -> {
                ...
                // Save a placeholder run first so that the FK on hrm_payroll_entry
                // is satisfied when entries are inserted in the next step.
                PayrollRun placeholder = PayrollRun.createPlaceholder(
                        runId, tenantId, orgId, agencyId, periode);

                return payrollRunRepository.save(placeholder)
                        .flatMap(savedPlaceholder ->
                            Flux.fromIterable(empList)
                                ...
                                .flatMap(entries -> {
                                    ...
                                    BigDecimal totalIrpp = entries.stream().map(PayrollEntry::irpp)
                                            .reduce(BigDecimal.ZERO, BigDecimal::add);
                                    BigDecimal totalCac = entries.stream().map(PayrollEntry::cac)
                                            .reduce(BigDecimal.ZERO, BigDecimal::add);
                                    BigDecimal totalCfc = entries.stream().map(PayrollEntry::cfc)
                                            .reduce(BigDecimal.ZERO, BigDecimal::add);

                                    PayrollRun finalRun = savedPlaceholder.withFinalTotals(
                                            totalBrut, totalNet, totalCnpsE, totalCnpsR,
                                            totalIrpp, totalCac, totalCfc, entries.size());

                                    return payrollRunRepository.save(finalRun)
                ...
                .flatMap(contract -> loanAdvanceRepository.findActiveByEmployeeId(tenantId, emp.id())
                        ...
                        .flatMap(totalLoanDeduction -> {
                            ...
                            PayrollEntry entry = PayrollEntry.create(tenantId, orgId, runId, emp.id(),
                                    salaireBase, brut, net, cnpsEmploye, cnpsEmployeur, irpp, cac, cfc,
    ...
    @Override
    public Flux<RunPayrollUseCase.MyPayslipSummary> getMyPayslips() {
        return ReactiveRequestContextHolder.getRequiredContext()
                .flatMapMany(context ->
                    employeeRepository.findByActorId(context.tenantId(), context.actorId())
                        .flatMapMany(emp ->
                            payrollEntryRepository.findByEmployeeId(context.tenantId(), emp.id())
                                .flatMap(entry ->
                                    payrollRunRepository.findById(context.tenantId(), entry.payrollRunId())
                                        .map(run -> new RunPayrollUseCase.MyPayslipSummary(
                                            entry.id(), run.id(), run.periode(), run.status().name(),
                                            entry.brut(), entry.net(), entry.cnpsEmploye(), entry.irpp(),
                                            entry.cac(), entry.cfc(),
                                            entry.paymentStatus().name(), entry.paymentChannel(),
                                            run.validatedAt()
                                        ))
                                )
                        )
    ...
    public Flux<PayslipLine> getMyPayslipLines(UUID entryId) {
        return ReactiveRequestContextHolder.getRequiredContext()
                .flatMapMany(context ->
                    employeeRepository.findByActorId(context.tenantId(), context.actorId())
                        .flatMapMany(emp ->
                            payrollEntryRepository.findById(context.tenantId(), entryId)
                                .filter(entry -> entry.employeeId().equals(emp.id()))
                                .switchIfEmpty(Mono.error(new IllegalArgumentException("Payslip entry not found or access denied")))
                                .flatMapMany(entry -> payslipLineRepository.findByPayrollEntryId(context.tenantId(), entryId))
                        )
                );
    }
