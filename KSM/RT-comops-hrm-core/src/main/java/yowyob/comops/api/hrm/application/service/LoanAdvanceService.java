...
public class LoanAdvanceService implements ManageLoanAdvanceUseCase {
    ...
    @Override
    public Flux<LoanAdvance> listAll() {
        return ReactiveRequestContextHolder.getRequiredContext()
                .flatMapMany(context -> {
                    if (context.organizationId() != null) {
                        return loanAdvanceRepository.findAllByOrganization(context.tenantId(), context.organizationId());
                    }
                    return loanAdvanceRepository.findAllByTenant(context.tenantId());
                });
    }

    @Override
    public Flux<LoanAdvance> getMyLoans() {
        return ReactiveRequestContextHolder.getRequiredContext()
                .flatMapMany(context ->
                        employeeRepository.findByActorId(context.tenantId(), context.actorId())
                                .flatMapMany(emp ->
                                        loanAdvanceRepository.findByEmployeeId(context.tenantId(), emp.id()))
                                .switchIfEmpty(Flux.empty()));
    }

    @Override
    public Mono<LoanAdvance> requestMyLoan(BigDecimal montant, int nbEcheances, String motif) {
        return ReactiveRequestContextHolder.getRequiredContext()
                .flatMap(context ->
                        employeeRepository.findByActorId(context.tenantId(), context.actorId())
                                .switchIfEmpty(Mono.error(new IllegalArgumentException("No employee record found for current user")))
                                .flatMap(emp -> {
                                    LoanAdvance loan = LoanAdvance.request(
                                            context.tenantId(), context.organizationId(), context.agencyId(),
