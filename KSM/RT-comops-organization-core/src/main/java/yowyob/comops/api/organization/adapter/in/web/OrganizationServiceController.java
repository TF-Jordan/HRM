...
public class OrganizationServiceController {
    ...
    private final ListPlatformServicePacksUseCase listPlatformServicePacksUseCase;
    private final ListCommercialSubscriptionCatalogUseCase listCommercialSubscriptionCatalogUseCase;
    ...
    private final ApplyOrganizationCommercialSubscriptionUseCase applyOrganizationCommercialSubscriptionUseCase;

    public OrganizationServiceController(ListPlatformServicesUseCase listPlatformServicesUseCase,
            ListPlatformServicePacksUseCase listPlatformServicePacksUseCase,
            ListCommercialSubscriptionCatalogUseCase listCommercialSubscriptionCatalogUseCase,
            ...
            ApplyOrganizationCommercialSubscriptionUseCase applyOrganizationCommercialSubscriptionUseCase) {
        this.listPlatformServicesUseCase = listPlatformServicesUseCase;
        this.listPlatformServicePacksUseCase = listPlatformServicePacksUseCase;
        this.listCommercialSubscriptionCatalogUseCase = listCommercialSubscriptionCatalogUseCase;
        ...
        this.applyOrganizationCommercialSubscriptionUseCase = applyOrganizationCommercialSubscriptionUseCase;
    }
    ...
    @GetMapping("/services/packs")
    @PreAuthorize("@businessAccessPolicy.hasUserContext(authentication)")
    public Mono<ResponseEntity<ApiResponse<List<OrganizationServicePackResponse>>>> listPacks() {
        return listPlatformServicePacksUseCase.listPlatformServicePacks()
                .map(OrganizationServicePackResponse::from)
                .collectList()
                .map(response -> ResponseEntity.ok(ApiResponse.success(response, "Platform service packs retrieved.")));
    }

    @GetMapping("/commercial-subscriptions/catalog")
    @PreAuthorize("@businessAccessPolicy.hasUserContext(authentication)")
    public Mono<ResponseEntity<ApiResponse<CommercialSubscriptionCatalogResponse>>> listCommercialSubscriptionCatalog() {
        return listCommercialSubscriptionCatalogUseCase.listCommercialSubscriptionCatalog()
                .map(CommercialSubscriptionCatalogResponse::from)
                .map(response -> ResponseEntity.ok(ApiResponse.success(response,
                        "Commercial subscription catalog retrieved.")));
    }
    ...
    @PostMapping("/{organizationId}/commercial-subscriptions")
    @PreAuthorize("@businessAccessPolicy.hasPermission(authentication, 'organizations:write')")
    public Mono<ResponseEntity<ApiResponse<OrganizationCommercialSubscriptionResponse>>> applyCommercialSubscription(
            @PathVariable("organizationId") UUID organizationId,
            @Valid @RequestBody Mono<ApplyCommercialSubscriptionRequest> requestMono) {
        return requestMono.zipWith(ReactiveRequestContextHolder.getRequiredContext())
                .flatMap(tuple -> applyOrganizationCommercialSubscriptionUseCase.applyOrganizationCommercialSubscription(
                        tuple.getT2().tenantId(),
                        organizationId,
                        tuple.getT1().planCode(),
                        tuple.getT1().addOnCodes()))
                .map(OrganizationCommercialSubscriptionResponse::from)
                .map(response -> ResponseEntity.ok(ApiResponse.success(response,
                        "Organization commercial subscription applied.")));
    }
